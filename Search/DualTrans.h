...
struct DualTrans {
    ...
    pair<double, long long> testDeltaNN(float d, vector<multiset<int>>& query_sets);
...
DualTrans::DualTrans(string path) {
    ...
    cout << "DualTrans construction time: " << construction_time_minutes << " minutes" << endl;
}

float DualTrans::get_size_in_MB() {
    float result = root->get_size_in_bytes();
    for(auto g : global_groups) {
        result += g.size() * 4 * 2;
    }
    return result / 1024.0f / 1024.0f;
}
...
vector<int> DualTrans::get_rep(multiset<int> trans) {
    ...
    for(int i = 0; i < dim; i++) {
        ...
        for(auto token : trans) {
            if(global_groups[i].find(token) != global_groups[i].end()) count1++;
            if(global_dual_groups[i].find(token) != global_dual_groups[i].end()) count2++;
        }
...
vector<set<int>> DualTrans::getGreedyGroups(map<int, int> token_freqs) {
    ...
    for(auto itr = freq_tokens.rbegin(); itr != freq_tokens.rend(); itr++) {
        ...
        for(int i = 0; i < (int)freq_counts.size(); i++) {
    ...
    while(candidates.size() > 0) {
        ...
        if(next->children.size() > 0) {
            ...
            for(auto item : next->data) {
                ...
                if(result.size() >= (size_t)k && sim < result.top().first) continue;
                result.push({sim, database[item.first]});
                if(result.size() > (size_t)k) result.pop();
            }
        }
        if(result.size() == (size_t)k && result.top().first >= candidates.top().first) break;
    }
    return check_count;
}
...
int DualTrans::findDeltaNN(multiset<int> trans, double delta) {
    ...
    int check_count = 0;
    while(candidates.size() > 0) {
        if(candidates.rbegin()->first < delta) break;
        ...
        if(next->children.size() > 0) {
            ...
            for(auto item : next->data) {
                check_count++;
                double sim = Measure::computeSim(trans, database[item.first]);
                if(sim >= delta) result.push_back(database[item.first]);
            }
        }
    }
    return check_count;
...
// Returns {avg_time_ms, avg_candidates_checked}
pair<double, long long> DualTrans::testDeltaNN(float d, vector<multiset<int>>& query_sets) {
